...
import { ContactEmailText } from "./enum";
...
export async function POST(req: Request) {
  try {
    ...
    await transporter.sendMail({
      ...
      subject: ContactEmailText.SUBJECT,
      html: htmlContent,
    });

    return NextResponse.json({
      success: true,
      message: ContactEmailText.SUCCESS_MESSAGE,
    });
  } catch (error) {
    console.error("Email error:", error);
    return NextResponse.json(
      { success: false, message: ContactEmailText.ERROR_MESSAGE },
      { status: 500 }
    );
  }
}
